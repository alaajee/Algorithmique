...
int main(int argc, char *argv[])
{
   ...
   perfstart(&p) ;
#endif
   long res_rec= EditDistance_NW_Rec(seq[0], length[0], seq[1], length[1]);
   long res_iter= EditDistance_NW_iter(seq[0], length[0], seq[1], length[1]);
   long res_aware= EditDistance_NW_Aware(seq[0], length[0], seq[1], length[1]);
   long res_oblivious= EditDistance_NW_Oblivious(seq[0], length[0], seq[1], length[1]);
#ifdef __PERF_MESURE__
   ...
   // printf("%ld\n", res_rec ) ; // print the distance on stdout
   // printf("%ld\n", res_iter ) ;
   // printf("%ld\n", res_aware ) ;
   printf("%ld\n", res_oblivious ) ;
   return 0 ;
