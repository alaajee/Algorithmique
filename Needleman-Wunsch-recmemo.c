...
long EditDistance_NW_Iter_1(struct NW_MemoContext *c,size_t i, size_t j , long *prev) {
    ...
    if (i == c->M) /* Reach end of X */
      {  if (j == c->N) res = 0;  /* Reach end of Y too */
         else res = (isBase(Yj) ? INSERTION_COST : 0) + prev[j+1-i+c->M] ;
      }
      else if (j == c->N) /* Reach end of Y but not end of X */
      {  res = (isBase(Xi) ? INSERTION_COST : 0) + prev[j-1-i+c->M];
      }
      else if (! isBase(Xi))  /* skip ccharacter in Xi that is not a base */
      {  ManageBaseError( Xi ) ;
         res = prev[j-i-1+c->M] ;
      ...
      {  /* Note that stopping conditions (i==M) and (j==N) are already stored in c->memo (cf EditDistance_NW_Rec) */ 
         long min = /* initialization  with cas 1*/
                   ...
                   + prev[j-i+c->M] ; 
         { long cas2 = INSERTION_COST + prev[j-i-1+c->M] ;      
           if (cas2 < min) min = cas2 ;
         }
         { long cas3 = INSERTION_COST + prev[j+1-i+c->M];      
           if (cas3 < min) min = cas3 ; 
         }
         res = min ;
      }
...
long EditDistance_NW_iter(char* A, size_t lengthA, char* B, size_t lengthB)
{
   ...
   long *prev = (long *)malloc((N+M)*sizeof(long));
   for (int i=M;i>=0 ;i--){
    for (int j=N;j>=0 ;j--){
        EditDistance_NW_Iter_1(&ctx,i,  j , prev) ;
    }
   }
   long res = prev[M];
   return res ;
...
long EditDistance_NW_CacheAware(struct NW_MemoContext *c,long *curr , long *prev)
{
   curr[c->M] = 0;
   prev[c->N] = 0;

   for (size_t k = c->M - 1; (int64_t) k >= 0; k--)
   {
      char Xk = c->X[k];
      curr[k] = (isBase(Xk) ? INSERTION_COST : 0) + curr[k + 1];
   }
   for (size_t l = c->N - 1; (int64_t) l >= 0; l--)
   {
      char Yl = c->Y[l];
      prev[l] = (isBase(Yl) ? INSERTION_COST : 0) + prev[l + 1];
   }

   for (size_t I = c->N; (int64_t) I >= 0; I -= BLOCK_SIZE_1)
   {
      size_t I_max = ((I >= BLOCK_SIZE_1) ? BLOCK_SIZE_1 : I);

      for (size_t J = c->M; (int64_t) J >= 0; J -= BLOCK_SIZE_2)
      {
         size_t J_Max = ((J >=BLOCK_SIZE_2) ?BLOCK_SIZE_2 : J);

         for (size_t k = J - 1; (int64_t)k >= (int64_t)(J - J_Max); k--)
         {
            char Xk = c->X[k];
            long res = prev[I];
            prev[I] = curr[k];

            for (size_t l = I - 1; (int64_t)l >= (int64_t)(I - I_max); l--)
            {
               char Yl = c->Y[l];
               ...
               {  
                  long min = (isUnknownBase(Xk) ? SUBSTITUTION_UNKNOWN_COST
                               : (isSameBase(Xk, Yl) ? 0 : SUBSTITUTION_COST)) + res;
                  min = (min > INSERTION_COST + prev[l] ? INSERTION_COST + prev[l] : min );
                  min = (min > INSERTION_COST + prev[l + 1] ? INSERTION_COST + prev[l + 1] : min);
...
long EditDistance_NW_Aware(char* A, size_t lengthA, char* B, size_t lengthB)
{
   ...
   size_t N = ctx.N ;
    ...
    long *prev = (long *) malloc ( (N+1) * sizeof(long)) ;
    long *curr = (long *) malloc ( (M+1) * sizeof(long)) ;                                                                                                                                                 
   
   long res = EditDistance_NW_CacheAware( &ctx,curr,prev);
...
static void EditDistance_NW_CacheOblivious(struct NW_MemoContext* c, size_t I, size_t J, size_t N, size_t M,long *prev,long *curr) {
    if ((N<= S) && (M<= S)) {
        for (size_t k = J - 1; (int64_t) k >= (int64_t)(J - M); k--) {
            char Xk = c->X[k];
            long res = prev[I];
            prev[I] = curr[k];

            for (size_t l = I - 1; (int64_t) l >= (int64_t)(I - N); l--) {
                char Yl = c->Y[l];
         ...
         (N> S) {
            EditDistance_NW_CacheOblivious(c, I, J, N/ 2, M,prev,curr);
            EditDistance_NW_CacheOblivious(c, I - N/ 2, J, N- N/ 2, M,prev,curr);
        } else  {
            EditDistance_NW_CacheOblivious(c, I, J, N, M/ 2,prev,curr);
            EditDistance_NW_CacheOblivious(c, I, J - M/ 2, N, M- M/ 2,prev,curr);
        }
...
long EditDistance_NW_Oblivious(char* A, size_t lengthA, char* B, size_t lengthB)
{
   ...
   for (size_t k = ctx.M - 1; (int64_t) k >= (int64_t) 0 ; k--)
   {
      char Xk = ctx.X[k];
      curr[k] = (isBase(Xk) ? INSERTION_COST : 0) + curr[k + 1] ;
   }
   for (size_t l = ctx.N - 1; (int64_t) l >= (int64_t) 0 ; l--)
   {
      char Yl = ctx.Y[l];
      prev[l] = (isBase(Yl) ? INSERTION_COST : 0) + prev[l + 1] ;
   }

   EditDistance_NW_CacheOblivious(&ctx, N, M,N, M,prev,curr);
